...
describe('get user group', () => {
    ...
    it('should throw error on incorrect _id field', async () => {
        try {
            await request.get(`${helpers.API_URL}/api/v1/merchants/${uuid.v4()}/user-group/123`, {
                json: true,
                headers: {
    ...
    it('should throw error if the document not exists', async () => {
        ...
        try {
            await request.get(`${helpers.API_URL}/api/v1/merchants/${uuid.v4()}/user-group/${uuid.v4()}`, {
                json: true,
                headers: {
                    'x-functions-key': process.env.X_FUNCTIONS_KEY
                }
    ...
    it('should create document when all validation passes', async () => {
        ...
        const user = await request.get(`${helpers.API_URL}/api/v1/merchants/${sampleUserGroup.merchantID}/user-group/${sampleUserGroup._id}`, {
            json: true,
            headers: {
                'x-functions-key': process.env.X_FUNCTIONS_KEY
            }
